...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        temp=self.learning_rate*(reward+self.gamma*np.max(self.qtable[next_state])-self.qtable[state,action] )
    ...
    for ep in tqdm(range(episode)):
        ...
        while True:
            ...
            if done:
                ...
                # print(count ,max_)
                if count!=15:
                    training_agent.qtable[next_state]=0
                if count>= max_:
                    max_=count
                    if os.path.exists("./Tables/taxi_table.npy"):
                        q=np.load("./Tables/taxi_table.npy")
                        flag=0
                        cnt=0
                        num=env.observation_space.n
                        total=num*0.5
                        for i in range(num):
                            if np.max(q[i,:])<np.max(training_agent.qtable[i,:]) or not q[i,:].any():
                                cnt+=1
                                if cnt>=total:
                                    flag=1
                                    break
