...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for i in range(self.belief.numRows):
            for j in range(self.belief.numCols):
                ...
                p=self.belief.getProb(i,j)*util.pdf(dis, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self) -> None:
        ...
        def z():
            return 0
        p_dic=collections.defaultdict(z)
        for (old,new) in self.transProb:
            p_dic[new]+=self.belief.getProb(old[0],old[1])*self.transProb[(old,new)]
        for i in range(self.belief.numRows):
            for j in range(self.belief.numCols):
                self.belief.setProb(i,j,p_dic[(i,j)])
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for po in self.particles:
            dis= ( (util.rowToY(po[0])-agentY)**2+ (util.rowToY(po[1])-agentX)**2)**0.5
            self.particles[po]*=util.pdf(dis, Const.SONAR_STD, observedDist)
        p = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            p[util.weightedRandomChoice(self.particles)] += 1
        self.particles=p
        # END_YOUR_CODE
        self.updateBelief()
    ...
    def elapseTime(self) -> None:
        ...
        for p in self.particles:
            for _ in range(self.particles[p]):
                new_p[util.weightedRandomChoice(self.transProbDict[p])]+=1
